...
<script>
  ...
  export default {
    props: ['clientId', 'database'],
    $_veeValidate: {
      validator: 'clientDatabase'
    },
    data () {
      return {
        dialog: false,
        headers: [
          ...
          {text: 'サーバー', value: 'server', sortable: false},
    ...
    methods: {
      ...
      deleteItem (item) {
        ...
        confirm('データベース情報を削除しますか?') && this.deleteDatabase(item.id, index)
      },
      close () {
        this.dialog = false
        setTimeout(() => {
      ...
      deleteDatabase (id, index) {
        let params = {}
        params['id'] = id
        axios.get('/api/database/delete', {
          params: params
        }).then((res) => {
          this.getClientDatabase()
        })
