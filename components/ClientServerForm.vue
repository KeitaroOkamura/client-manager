...
<script>
  ...
  export default {
    props: ['clientId', 'server'],
    $_veeValidate: {
      validator: 'clientServer'
    },
    data () {
      return {
        dialog: false,
    ...
    methods: {
      ...
      deleteItem (item) {
        ...
        confirm('サーバー情報を削除しますか?') && this.deleteServer(item.id, index)
      },
      close () {
        this.dialog = false
        setTimeout(() => {
      ...
      updateServer () {
        ...
        axios.get('/api/server/update', {
          params: params
        }).then((res) => {
          this.getClientServer()
        })
      },
      deleteServer (id, index) {
        let params = {}
        params['id'] = id
        axios.get('/api/server/delete', {
          params: params
        }).then((res) => {
          this.getClientServer()
        })
