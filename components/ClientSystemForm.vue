...
<script>
  ...
  export default {
    props: ['clientId', 'system'],
    $_veeValidate: {
      validator: 'clientSystem'
    },
    data () {
      return {
        dialog: false,
    ...
    methods: {
      ...
      deleteItem (item) {
        ...
        confirm('システム情報を削除しますか?') && this.deleteSystem(item.id, index)
      },
      close () {
        this.dialog = false
        setTimeout(() => {
      ...
      insertSystem () {
        ...
        axios.get('/api/system/add', {
          params: params
        }).then((res) => {
          this.getClientSystem()
        })
      },
      updateSystem () {
        let params = {}
        ...
        axios.get('/api/system/update', {
          params: params
        }).then((res) => {
          this.getClientSystem()
        })
      },
      deleteSystem (id, index) {
        let params = {}
        params['id'] = id
        axios.get('/api/system/delete', {
          params: params
        }).then((res) => {
          this.getClientSystem()
        })
